...
generate.post("/billing/:id/:type", async (c) => {
    ...
    // Fetch Data
    const period = db.query("SELECT * FROM billing_periods WHERE id = ?").get(billingPeriodId);
    if (!period)
    ...
    const company = db.query("SELECT * FROM companies WHERE id = ?").get(project.company_id);
    const rows = db.query(`
    ...
    try {
        ...
        // Record in DB
        db.run("INSERT INTO generated_documents (billing_period_id, type, file_path) VALUES (?, ?, ?)", [billingPeriodId, type, filename]);
